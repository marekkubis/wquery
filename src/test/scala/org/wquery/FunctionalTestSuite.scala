...
class FunctionalTestSuite extends WQueryTestSuite {
  ...
  // Q56a
  @Test def countTheNumberOfEdgesPerNounSynset() = result of "count({}[pos = `n`]._&synset)/count({}[pos = `n`])" should startWith ("0.989")
  ...
  // Q59
  @Test def countTheNumberOfSemanticRelationsSuccessorsPerNonLeafSynset() = result of "count({}[not empty(^hypernym)]._&synset)/count({}[not empty(^hypernym)])" should startWith ("1.176")
